...
def draw_solution(grid, silver_cells, gold_cells, file_name):

    width = 10
    grid_nodes = [[[i, j], [i + 1, j], [i + 1, j + 1], [i, j + 1], [i, j]]
                  for i in range(grid[0]) for j in range(grid[1])]
    silver_blocks = [[[i, j], [i + 1, j], [i + 1, j + 1], [i, j + 1], [i, j]] for (i ,j) in silver_cells]
    gold_blocks = [[[i, j], [i + 1, j], [i + 1, j + 1], [i, j + 1], [i, j]] for (i ,j) in gold_cells]
    fig, ax = plt.subplots()

    gold_blocks = sorted(gold_blocks, key=functools.cmp_to_key(compare))
    _draw_blocks(grid_nodes, width, ax, 1, 'w')
    _draw_blocks(gold_blocks, width, ax, 2, 'r')
    _draw_blocks(silver_blocks, width, ax, 2, 'grey')
    _draw_block_numbers(gold_blocks, width, fig, 2, 'black')
